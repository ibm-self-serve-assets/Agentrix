...
class MaximoTool:
    ...
    def update_schedule(self, wonum: str, sched_start: str, sched_finish: str) -> str:
        ...
        if response.status_code in [200,204]:
            return f"Schedule dates successfully updated for work order '{wonum}'."

        elif response.status_code == 400:
            return f"Bad request: Please check the payload. Possible issues with schedule date format or work order '{wonum}'."

        elif response.status_code == 401:
            return "Unauthorized: Invalid API key or authentication error."

        elif response.status_code == 403:
            return "Forbidden: You do not have permission to update schedule dates for this work order."

        elif response.status_code == 404:
            return f"Work order '{wonum}' not found or URL is invalid."

        elif response.status_code >= 500:
            return f"Server error ({response.status_code}): Maximo backend is unavailable or encountered an error during schedule update."

        else:
            return f"Unexpected error. Status code: {response.status_code}, Response: {response.text}"
...
def update_workorder_tool(query: str) -> StringToolOutput:
    ...
    try:
        ...
        print("schedule dates = ",sched_start,sched_finish)
        maximo = MaximoTool()

...
#     print("response from maximo workorder detail api = ",coords)
#     description=coords['description']
#     logger.info(f"Response from Maximo workorder detail API = {coords}")
...
#     "Work order {wonum} has the description: <Description>."
#     """
...
#     For work order **{wonum}**, extract the item number from the provided inventory summary, 
#     retrieve the store location for the item, and add the item to the work order in Maximo.
#     """
#     summary = inventory_summary

#     print("final summary after all agents = ",summary)

#     invfilteragent_response=agent_maximo.getItemDetails(summary)
#     itemnum=invfilteragent_response.get('itemnum')
#     if itemnum is not None:
#         itemresponse=agent_maximo.get_item_url(itemnum)
#         #invsummary=''.join([m.text for m in invfilteragent_response.result.content])
#         print("final inv  summary after all agents = ",invfilteragent_response)
#         location=itemresponse['storeloc']
#         additem=agent_maximo.addItem(itemnum,location,wonum,siteid='BEDFORD')
   
#         if additem:
#             response=f"""
# Inventory Agent

# Based on the reported issue: **{description}**, the following materials have been identified and are available in your inventory:

# {invfilteragent_response}

# These materials have now been successfully added to your work order **{wonum}**.
# You can proceed with the necessary repairs or maintenance using the provided materials.
# """
#             print(response)
#             summary=response
#             return summary
#         else:
#             print("Items are not available in your Inventory. Inventory cannot be planned")
#             summary="Items are not available in your Inventory. Inventory cannot be planned"
#             return summary
#     else:
...
async def generate(input_text: str) -> str:
    ...
    from src.agents.travel_planner_agent import get_beeai_framework_agent, observer
    from src.agents.maximo_agent import AgentMaximo

    # Extract number after "wonum"
    match = re.search(r'\bwonum\s+(\d+)', prompt, re.IGNORECASE)
    if match:
        wonum = match.group(1)
    else:
        wonum = None
...
"""
    ...
    summary = ''.join([m.text for m in inventory_response.result.content])
